...
class Player:
    def handEvaluator(self,hand):
        ...
        for i in range(0, 4):
            if(numSuits[i] >= 2):
                points = points + 300 - 50*numSuits[i]
    ...
    def valueToSize(self,value,pot,threshold):
        trueSize = float(value)/pot
        closest = 0
        closestDifference = trueSize
        for size in threshold:
    ...
    def run(self, input_socket):
        ...
        while True:
            ...
            if dataType == "GETACTION":
                ...
                hasNotActed = True
                if button:
                    p = historian.exploitProbability(pot, actionNumber, legalActions, lastActions)
                    print "p is "+str(p)
                    historian.update(pot, actionNumber, lastActions[-1])
